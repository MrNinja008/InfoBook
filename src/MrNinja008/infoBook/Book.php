...
class Book extends PluginBase implements Listener {
    ...
    public function onJoin(PlayerJoinEvent $event) {
        ...
        if(!$config->exists($player->getName())){
            ...
            foreach ($contents as $index=>$content) $book->setPageText($index, $this->replaceTags($content[$index], $player));
    ...
    {
        return TextFormat::colorize(str_replace(["{max_players}", "{online_players}", "{player}","{ping}" ], [$this->getServer()->getMaxPlayers(), (string) count($this->getServer()->getOnlinePlayers()), $player->getName(), (string) $player->getPing()], $string));
    }
}
